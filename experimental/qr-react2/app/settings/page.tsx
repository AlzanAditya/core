...
import { getAllProducts, getAllScanLogs, Product, ScanLog } from "@/lib/qr-api"
import { QR_CONFIG } from "@/lib/qr-config"

export default function SettingsPage() {
  const [productsData, setProductsData] = useState<Product[]>([])
  const [transactionsData, setTransactionsData] = useState<ScanLog[]>([])
  ...
  useEffect(() => {
    async function loadExportData() {
      try {
        ...
        try {
          setTransactionsData(await getAllScanLogs())
        } catch (err) {
          setDataError(err instanceof Error ? err.message : "Scan log membutuhkan session admin.")
        }
      } catch (err) {
        setDataError(err instanceof Error ? err.message : "Gagal memuat data ekspor.")
      }
    }
    loadExportData()
  }, [])
    ...
    <Tabs defaultValue="general" className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label>Nama Aplikasi</Label><Input defaultValue={QR_CONFIG.APP_NAME} className="bg-secondary" readOnly /></div>
              <div className="space-y-2"><Label>Versi</Label><Input defaultValue={QR_CONFIG.APP_VERSION} className="bg-secondary" readOnly /></div>
            </div>
            <div className="space-y-2"><Label>Worker URL</Label><Input defaultValue={QR_CONFIG.WORKER_URL} className="bg-secondary" readOnly /></div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label>Storage Bucket</Label><Input defaultValue={QR_CONFIG.STORAGE_BUCKET} className="bg-secondary" readOnly /></div>
              <div className="space-y-2"><Label>Supabase URL</Label><Input defaultValue={QR_CONFIG.SUPABASE_URL} className="bg-secondary" readOnly /></div>
            </div>
      ...
      <TabsContent value="data" className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            {dataError && <p className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">{dataError}</p>}
            <div className="space-y-3"><div><Label>Ekspor Data Produk</Label><p className="text-xs text-muted-foreground">{productsData.length} produk dari Supabase</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => exportToCSV(productsData as unknown as Record<string, unknown>[], "products")}><FileSpreadsheet className="mr-2 h-4 w-4" />CSV</Button><Button variant="outline" size="sm" onClick={() => exportToJSON(productsData as unknown as Record<string, unknown>[], "products")}><FileText className="mr-2 h-4 w-4" />JSON</Button></div></div>
            <Separator />
            <div className="space-y-3"><div><Label>Ekspor Data Transaksi</Label><p className="text-xs text-muted-foreground">{transactionsData.length} scan log dari Supabase</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => exportToCSV(transactionsData as unknown as Record<string, unknown>[], "transactions")}><FileSpreadsheet className="mr-2 h-4 w-4" />CSV</Button><Button variant="outline" size="sm" onClick={() => exportToJSON(transactionsData as unknown as Record<string, unknown>[], "transactions")}><FileText className="mr-2 h-4 w-4" />JSON</Button></div></div>
            <Separator />
            <div className="space-y-3"><div><Label>Full Backup</Label><p className="text-xs text-muted-foreground">Download semua data live</p></div><Button size="sm" onClick={() => { const allData = { products: productsData, transactions: transactionsData }; exportToJSON([allData], "full_backup") }}><Download className="mr-2 h-4 w-4" />Download</Button></div>
            <Separator />
    ...
    <Tabs defaultValue="general" className="space-y-2.5">
      ...
      <TabsContent value="general" className="space-y-2.5">
        <Card>
          ...
          <CardContent className="p-2.5 pt-0 space-y-2">
            <div className="space-y-1"><Label className="text-[10px]">Nama</Label><Input defaultValue={QR_CONFIG.APP_NAME} className="h-8 bg-secondary text-xs" readOnly /></div>
            <div className="space-y-1"><Label className="text-[10px]">Versi</Label><Input defaultValue={QR_CONFIG.APP_VERSION} className="h-8 bg-secondary text-xs" readOnly /></div>
            <div className="space-y-1"><Label className="text-[10px]">Worker</Label><Input defaultValue={QR_CONFIG.WORKER_URL} className="h-8 bg-secondary text-xs" readOnly /></div>
      ...
      <TabsContent value="data" className="space-y-2.5">
        <Card>
          ...
          <CardContent className="p-2.5 pt-0 space-y-2">
            {dataError && <p className="rounded-md border border-destructive/40 bg-destructive/10 p-2 text-[10px] text-destructive">{dataError}</p>}
            <div className="space-y-1"><span className="text-[11px] text-muted-foreground">Produk ({productsData.length})</span><div className="flex gap-1.5"><Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" onClick={() => exportToCSV(productsData as unknown as Record<string, unknown>[], "products")}>CSV</Button><Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" onClick={() => exportToJSON(productsData as unknown as Record<string, unknown>[], "products")}>JSON</Button></div></div>
            <Separator />
            <div className="space-y-1"><span className="text-[11px] text-muted-foreground">Transaksi ({transactionsData.length})</span><div className="flex gap-1.5"><Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" onClick={() => exportToCSV(transactionsData as unknown as Record<string, unknown>[], "transactions")}>CSV</Button><Button variant="outline" size="sm" className="h-7 text-[10px] flex-1" onClick={() => exportToJSON(transactionsData as unknown as Record<string, unknown>[], "transactions")}>JSON</Button></div></div>
            <Separator />
